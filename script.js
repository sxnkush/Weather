...
async function fetchDataLocation(location) {
  const [city, state_code = "", country_code = ""] = location
    .split(",")
    .map((item) => item.trim());
  const country = await fetch(
    `https://api.openweathermap.org/geo/1.0/direct?q=${city},${state_code},${country_code}&limit=30&appid=bb4da9c6051404c62d9d30025b8b91be`
  );
  const ans = await country.json();
  const lat = ans[0].lat;
  const lon = ans[0].lon;
  return { ans: ans, lat: lat, lon: lon };
}

async function fetchTemp(location) {
  let dataLocation = await fetchDataLocation(location);
  const response = await fetch(
    `https://api.openweathermap.org/data/2.5/weather?lat=${dataLocation.lat}&lon=${dataLocation.lon}&appid=${API_key}&units=metric`
  );
...
async function fetchPollution(location) {
  ...
  const poll = await fetch(
    `https://api.openweathermap.org/data/2.5/air_pollution?lat=${dataLocation.lat}&lon=${dataLocation.lon}&appid=${API_key}&units=metric`
  );
...
async function foreCast(location) {
  let dataLocation = await fetchDataLocation(location);
  const forecast_5 = await fetch(
    `https://api.openweathermap.org/data/2.5/forecast?lat=${dataLocation.lat}&lon=${dataLocation.lon}&appid=${API_key}&units=metric`
  );
  ...
  let gap =
    Number.parseInt(ans_forecast.list[0].dt_txt.split(" ")[1].split(":")[0]) /
    3;
  let count = 0;
  for (let i = gap; i < 40; i++) {
    if (ans_forecast.list[i].dt_txt.split(" ")[1] == "12:00:00") {
      ...
      if (
        ans_forecast.list[i].weather[0].description.match(/sunny/i) ||
        ans_forecast.list[i].weather[0].description.match(/clear/i)
      ) {
        fore.innerHTML += `<img src = "./sunny.svg"><p>${ans_forecast.list[i].weather[0].description}</p>`;
      } else if (
        ans_forecast.list[i].weather[0].description.match(/rain/i) ||
        ans_forecast.list[i].weather[0].description.match(/thunderstorm/i)
      ) {
...
function addHTMLforecast(days, day) {
  let forecast_class = document.getElementsByClassName("forecast")[0];
  for (let i = 0; i < 5; i++) {
    if (day < 7) {
...
async function getTemp() {
  ...
  let temp = await fetchTemp(location);
  let air_poll = await fetchPollution(location);
  let dataLocation = await fetchDataLocation(location);
  let city_class = document.getElementsByClassName("city")[0];
  city_class.innerHTML = `<h1>${temp.name}</h1><br><p>${dataLocation.ans[0].state}</p> <p>${temp.sys.country}</p>`;

  let d = new Date();
  let day = d.getDay();
  let days = ["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"];
  let date_class = document.getElementsByClassName("date")[0];
  date_class.innerHTML = `${days[day - 1]} ${temp.main.temp_max}&deg/${
    temp.main.temp_min
  }&deg`;
  ...
  if (
    temp.weather[0].description.match(/sunny/i) ||
    temp.weather[0].description.match(/clear/i)
  ) {
    temperature_class.innerHTML = `<img src = "./sunny.svg"><h1>${temp.main.temp}&degC</h2>`;
  } else if (
    temp.weather[0].description.match(/rain/i) ||
    temp.weather[0].description.match(/thunderstorm/i)
  ) {
  ...
  let weather_des =
    ...
    temp.weather[0].description.slice(1);
  description_class.innerHTML = `<p>${weather_des}</p><p>Air Quality: ${aqi}`;
  
  await addHTMLforecast(days, day);
  foreCast(location);
  //CSS
