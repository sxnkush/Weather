...
const getTemp = async() => {
    ...
    const poll = await fetch(`http://api.openweathermap.org/data/2.5/air_pollution?lat=${lat}&lon=${lon}&appid=${API_key}&units=metric`)
    const air_poll = await poll.json()
    console.log(air_poll.list[0].main.aqi)

    const forecast_5 = await fetch(`http://api.openweathermap.org/data/2.5/forecast?lat=${lat}&lon=${lon}&appid=${API_key}`)
